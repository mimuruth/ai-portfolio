/**
 * Build an ILLUSTRATED-COVER copy of the textbook: identical interior, but the
 * cover carries a modern generative "AI" artwork (a neural-mesh illustration).
 * Output: ai-engineering-textbook-illustrated.pdf  (generated locally; not committed).
 *
 *   node build-art.mjs
 */
...
// ---------------------------------------------------------------------------
// Generative cover artwork: a deterministic neural-mesh over a gradient field.
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// Interior: identical to the standard build.
// ---------------------------------------------------------------------------
...
<head>
<meta charset="utf-8">
<title>AI Engineering from Scratch to Shipped</title>
<style>${hljsCss}</style>
<style>${bookCss}</style>
<style>${artCss}</style>
</head>
...
<section class="cover cover-art">
  ...
  <div class="art-text">
    <div class="art-kicker">A Practical Textbook &amp; Portfolio Guide</div>
...
writeFileSync(join(__dirname, "book-art.html"), html);

const browser = await puppeteer.launch({ headless: "new", args: ["--no-sandbox"] });
const page = await browser.newPage();
await page.goto(pathToFileURL(join(__dirname, "book-art.html")).href, { waitUntil: "networkidle0", timeout: 120000 });
